/*
	...
	Can be compiled with the following command
		gcc -Wall -W -posix -pedantic -O2 -g -o /usr/local/bin/dirr dirr.c
	or
...
#define VERSION \
        "DIRR v2.38 copyright (C) 1992,1999 Bisqwit (http://iki.fi/bisqwit/)\n" \
...
static void SetDefaultOptions(void)
{
	...
	strcpy(BlkStr, "<BLKDEV>");	/* Modify with -db */
	strcpy(ChrStr, "<CHRDEV>");	/* Modify with -dc */
}
...
int GetDescrColor(const char *descr, int index);
...
char *GetError(int errno)
{
	...
	strncpy(Buffer, strerror(errno), 63);
	Buffer[63] = 0;
	a=strlen(Buffer);
	while(a && Buffer[a-1]=='\n')Buffer[--a]=0;
	return Buffer;
...
char *GetSet(char **S, const char *name)
{
    char *s = *S;
    int namelen;
...
int GetModeColor(const char *text, int Chr)
{
    int Char;
    int Dfl = 7;
...
int GetNameAttr(const struct stat *Stat, const char *fn)
{
...
static void Puuhun(struct stat *Stat, char *Name
#ifdef DJGPP
	, unsigned int attr
#endif
	)
...
static void SingleFile(char *Buffer, struct stat *Stat)
{
    #ifndef S_ISLNK
    int Links=0; /* hack */
    #endif
