...
Cases::Cases (const Span * span, uint32_t span_size)
	: def (span_size == 0 ? nullptr : span[span_size - 1].to)
	...
	: branches ()
{
	for (;;)
	{
		...
		if (n == 1)
		{
			if (next == nullptr || s[0].to != next)
			{
				branches.push_back (std::make_pair (static_cast<const Cond *> (nullptr), s[0].to));
			}
			return;
		}
		else if (n == 2 && bg == next)
	...
	, lgo (nullptr)
{
	...
	// must be nullptr to trigger explicit goto generation in linear 'if'
	hgo = hSpans == 0
		? nullptr
		: new SwitchIf (hspan, hSpans, lgo ? nullptr : next);
...
Cpgoto::Cpgoto (const Span * span, uint32_t nSpans, const Span * hspan, uint32_t hSpans, const State * next)
	: hgo (hSpans == 0 ? nullptr : new SwitchIf (hspan, hSpans, next))
...
void Go::init (const State * from)
{
	...
	const State * bitmap_state = nullptr;
	for (uint32_t i = 0; i < nSpans; ++i)
	{
		if (span[i].to->isBase)
		{
