...
RegExp *Scanner::schr(uint32_t c)
{
...
RegExp *Scanner::ichr(uint32_t c)
{
	if (is_alpha(c)) {
...
RegExp * Scanner::mkDiff (RegExp * e1, RegExp * e2)
{
...
RegExp * Scanner::mkDot()
{
	//Range * full = opts->encoding.fullRange();
	Range* full = Range::ran(0, 0x100);
	uint32_t c = '\n';
	//if (!opts->encoding.encode(c))
	//	fatalf("Bad code point: '0x%X'", c);
 ...
 */
RegExp * Scanner::mkDefault()
{
	Range * def = Range::ran (0, 0x100/*opts->encoding.nCodeUnits()*/);
